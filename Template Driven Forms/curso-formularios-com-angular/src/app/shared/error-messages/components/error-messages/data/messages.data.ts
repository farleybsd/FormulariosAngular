...
export const errorMessages: TErrorMessages['errorMessages'] = {
  ...
  passwordAreNotEqual: ({ field1, field2 }) =>
    `A senha do campo "${field1}" nao e igual a do campo "${field2}" `,
  isNicknameTaken: () => 'Nickname já está em uso',
  minlength: ({ requiredLength, actualLength }) =>
    `O Valor deve conter no minino ${requiredLength} caracteres (atualmente ${actualLength})`,
  invalidMaxNames: ({ maxQuantity, currentQuantity }) =>
    `Quantidade maxima de nomes (${maxQuantity}) (atualmente ${currentQuantity})`,
